...
import { setActiveWorkspace, deleteWorkspaceById, fetchWorkspaces } from "@/store/workspaces-slice";
...
export default function WorkspaceSwitcher({
  ...
  const handleDelete = async (id: number, name: string) => {
    ...
    setDeletingId(id);
    try {
      await dispatch(deleteWorkspaceById({ id, token })).unwrap();
      const result = await dispatch(fetchWorkspaces(token)).unwrap();
      toast.success("Workspace deleted", { description: name });
      if (activeWorkspaceId === id && result.length > 0) {
        const next = result[0];
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger className="flex w-full items-center gap-2 rounded-lg px-2 py-1.5 text-left transition-colors hover:bg-muted">
        <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-md text-sm" style={{ backgroundColor: activeWorkspace.accentColor + "20", color: activeWorkspace.accentColor }}>
      ...
      <DropdownMenuContent className="w-56">
        ...
        {workspaces.map((ws) => (
          ...
          >
            <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded text-xs" style={{ backgroundColor: ws.accentColor + "20", color: ws.accentColor }}>
              {ws.icon}
            </span>
